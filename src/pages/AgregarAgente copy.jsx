...
//const API_URL = "http://192.168.9.115:4000"
export default function AgregarAgente({
  user,
  reload,
  onClose
}) {
  ...
  const handleBuscarPersona = async () => {
    ...
    if (form.numero_documento.length < 8 || form.numero_documento.length > 10) {
    return
  }
...
<motion.button
  // Desactiva las animaciones de hover/tap por completo si está buscando o la longitud es inválida
  whileHover={
    searchingDni || form.numero_documento.length < 8 || form.numero_documento.length > 10
      ? {}
      : { scale: 1.03, y: -1 }
  }
  whileTap={
  ...
  disabled={
    ...
    form.numero_documento.length > 10
  }
  className="
    ...
    /* CAMBIOS AQUÍ: Manejo correcto del cursor y opacidad */
    disabled:opacity-50 
