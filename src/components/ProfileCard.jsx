...
// Helper: reads user data from localStorage and formats name/title
const getUserInfoFromStorage = () => {
  try {
    ...
    const nro_doc = user?.nro_doc || "";
    const tipo_usuario = user?.tipo_usuario || "";

    // Formato: "Nombre N. Apellido"
    const nombresArray = nombres.trim().split(/\s+/);
const apellidosArray = apellidos.trim().split(/\s+/);

const primerNombre = nombresArray[0] || "";
const inicialSegundoNombre = nombresArray[1]
  ? `${nombresArray[1].charAt(0).toUpperCase()}.`
  : "";

const primerApellido = apellidosArray[0] || "";

const formattedName =
  [primerNombre, inicialSegundoNombre, primerApellido]
    .filter(Boolean)
    .join(" ") || "Usuario";
    
    // Formato: "71480222 · Sistemas"
    const formattedTitle = [nro_doc, tipo_usuario]
      .filter(Boolean)
      .join(" · ");
...
const ProfileCardComponent = ({
  ...
  // Read from localStorage; fall back to props if localStorage is empty
  const { formattedName, formattedTitle } = getUserInfoFromStorage();
  const name = formattedName || nameProp || 'Javi A. Torres';
  const title = formattedTitle || titleProp || 'Software Engineer';
